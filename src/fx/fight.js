...
(function (F) {
   ...
   * line, the two strongest horizontals in the frame. At 118 they ran
   * straight through both fighters' shoulders in every single screenshot.
   * The band from the head down to the guard is where the readable action
   * is, and it has to be free of long horizontals. */
  var WALL_TOP = 96;
  ...
  Match.prototype.drive = function (f, c, foe) {
    ...
    /* Jump velocity is set by the frame, not by taste. Gravity is 1500 and
     * the apex is v^2/2g, so -490 gives 80 units of lift and 39 frames of
     * airtime -- and 80 is what the framing allows: the floor is 216, the
     * fighter is 125 tall, and the head has to stay inside the frame. A
     * commercial fighter jumps two body heights because its camera is far
     * wider relative to the character than this one is. */
    if (c.u) { f.start('jump'); f.vy = -490; f.onGround = false; f.vx = (fwd ? f.facing : back ? -f.facing : 0) * 118;
  ...
  Match.prototype.render = function () {
    ...
    if (sup) {
      ...
      /* A trail off the striking hand. The super's whole visual identity
       * was a slightly bigger spark; a smear that follows the fist is what
       * actually says "this is the big one". */
      var sup2 = sup.pose();
      var swr = sup2.arms[1].wrist, sel = sup2.arms[1].elbow;
      for (var q4 = 0; q4 < 7; q4++) {
        var tq = q4 / 6;
        var lx2 = sel[0] + (swr[0] - sel[0]) * (1 + tq * 0.55);
        var ly2 = sel[1] + (swr[1] - sel[1]) * (1 + tq * 0.55);
        surf.addDisc(lx2 * S + pan, ly2 * S + oy, (9 - q4) * S,
          tc2[0], tc2[1], tc2[2], 0.13 * (1 - tq) * k2);
      }
  ...
  Match.prototype.hud = function (surf) {
    ...
    var bw = 128 * S, bh = 11 * S, by = 12 * S, mg = 12 * S;
    ...
    var bgap = 33 * S;                       // room for the portrait plate
    bar(surf, mg + bgap, by, bw, bh, ha, ca[0], ca[1], ca[2], false, this.ghostA);
    bar(surf, w - mg - bgap - bw, by, bw, bh, hb, cb2[0], cb2[1], cb2[2], true, this.ghostB);
    // super meters
    meterBar(surf, mg + bgap + 4 * S, by + bh + 5 * S, bw * 0.56, 4.5 * S, this.a.meter / 100, false, this.tHud);
    meterBar(surf, w - mg - bgap - 4 * S - bw * 0.56, by + bh + 5 * S, bw * 0.56, 4.5 * S,
      this.b.meter / 100, true, this.tHud);
    if (this.a.meter >= 100) text(surf, 'MAX', mg + bgap + 4 * S + bw * 0.56 + 5 * S, by + bh + 5 * S,
      1.5 * S, F.rgb(255, 236, 170), F.rgb(20, 10, 8));
    if (this.b.meter >= 100) text(surf, 'MAX', w - mg - bgap - 4 * S - bw * 0.56 - 5 * S - textW('MAX', 1.5 * S),
      by + bh + 5 * S, 1.5 * S, F.rgb(255, 236, 170), F.rgb(20, 10, 8));

    /* Portraits. The single thing that most makes a fighting-game HUD read
     * as one, and the head is already solved every frame -- so it is the
     * live head, blinking and flinching, not a static picture. */
    var pw = 26 * S, ph = 26 * S, pty = by - 3 * S;
    for (var pi = 0; pi < 2; pi++) {
      var pf = pi ? this.b : this.a;
      var pxx = pi ? w - mg - pw : mg;
      surf.fillRect(pxx - 2 * S, pty - 2 * S, pw + 4 * S, ph + 4 * S, F.rgb(12, 10, 15));
      surf.fillRect(pxx - 1 * S, pty - 1 * S, pw + 2 * S, ph + 2 * S, F.rgb(92, 82, 88));
      surf.gradientV(pxx, pty, pw, ph, F.rgb(46, 38, 48), F.rgb(20, 16, 24));
      F.drawPortrait(this.sh, pf, pxx + pw * 0.5, pty + ph * 0.5, S * 1.15, pw, ph);
      // a lit lower edge, so the plate has a thickness
      surf.blendRect(pxx, pty + ph - 1.2 * S, pw, 1.2 * S, 190, 158, 120, 0.45);
    }

    var np = 1.5 * S;
    text(surf, this.a.name, mg + pw + 5 * S, by + bh + 11 * S, np, F.rgb(240, 226, 200), F.rgb(10, 8, 12));
    var nb = this.b.name;
    text(surf, nb, w - mg - pw - 5 * S - textW(nb, np), by + bh + 11 * S, np,
      F.rgb(240, 226, 200), F.rgb(10, 8, 12));
    ...
    for (var i = 0; i < 2; i++) {
      for (var k = 0; k < 2; k++) {
        var px = i ? w - mg - pw - 12 * S - k * 11 * S : mg + pw + 5 * S + k * 11 * S;
